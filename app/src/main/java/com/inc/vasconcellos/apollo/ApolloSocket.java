...
public class ApolloSocket extends Emitter {

    private static final Logger logger = Logger.getLogger(ApolloSocket.class.getName());
    ...
    public static final String EVENT_NETWORK_OFFLINE = ApolloSocketManager.EVENT_NETWORK_OFFLINE;

    public static final String EVENT_NETWORK_ONLINE = ApolloSocketManager.EVENT_NETWORK_ONLINE;
    ...
    private int ids;
    private String nsp;
    private ApolloSocketManager io;
    ...
    public ApolloSocket(ApolloSocketManager io, String nsp) {
        this.io = io;
        this.nsp = nsp;
    }
    ...
    private void subEvents() {
        ...
        ApolloSocket.this.subs = new LinkedList<On.Handle>() {{
            add(On.on(ApolloSocket.this.io, Manager.EVENT_OPEN, new Listener() {
                @Override
                public void call(Object... args) {
                    ApolloSocket.this.onopen();
                }
            }));
            add(On.on(ApolloSocket.this.io, Manager.EVENT_PACKET, new Listener() {
                @Override
                public void call(Object... args) {
                    ApolloSocket.this.onpacket((Packet<JSONArray>) args[0]);
                }
            }));
            add(On.on(ApolloSocket.this.io, Manager.EVENT_CLOSE, new Listener() {
                @Override
                public void call(Object... args) {
                    ApolloSocket.this.onclose(args.length > 0 ? (String) args[0] : null);
                }
            }));
            add(On.on(ApolloSocket.this.io, Manager.E, new Listener() {
                @Override
                public void call(Object... args) {
                    ApolloSocket.this.onclose(args.length > 0 ? (String) args[0] : null);
                }
     ...
     */
    public ApolloSocket open() {
        EventThread.exec(new Runnable() {
            @Override
            public void run() {
                if (ApolloSocket.this.connected) return;

                ApolloSocket.this.subEvents();
                ApolloSocket.this.io.open(); // ensure open
                if (ApolloSocketManager.ConnectionState.OPEN == ApolloSocket.this.io.connectionState) ApolloSocket.this.onopen();
            }
        });
        return this;
    }
     ...
     */
    public ApolloSocket send(final Object... args) {
        EventThread.exec(new Runnable() {
            @Override
            public void run() {
                ApolloSocket.this.emit(EVENT_MESSAGE, args);
            }
        });
        return this;
    }
        ...
        EventThread.exec(new Runnable() {
            ...
            public void run() {
                if (events.containsKey(event)) {
                    ApolloSocket.super.emit(event, args);
                    return;
                }
                ...
                if (_args.get(_args.size() - 1) instanceof Ack) {
                    logger.fine(String.format("emitting packet with ack id %d", ApolloSocket.this.ids));
                    ApolloSocket.this.acks.put(ApolloSocket.this.ids, (Ack)_args.remove(_args.size() - 1));
                    jsonArgs = remove(jsonArgs, jsonArgs.length() - 1);
                    packet.data = jsonArgs;
                    packet.id = ApolloSocket.this.ids++;
                }

                if (ApolloSocket.this.connected) {
                    ApolloSocket.this.packet(packet);
                } else {
                    ApolloSocket.this.sendBuffer.add(packet);
                }
            }
        });
        return this;
    ...
    public Emitter emit(final String event, final Object[] args, final Ack ack) {
        EventThread.exec(new Runnable() {
            ...
            public void run() {
                ...
                ApolloSocket.this.acks.put(ids, ack);
                packet.id = ids++;
     ...
     * @return a reference to this object.
     */
    public ApolloSocket close() {
        EventThread.exec(new Runnable() {
            @Override
            public void run() {
                if (ApolloSocket.this.connected) {
                    logger.fine(String.format("performing disconnect (%s)", ApolloSocket.this.nsp));
                    ApolloSocket.this.packet(new Packet<JSONArray>(Parser.DISCONNECT));
                }

                ApolloSocket.this.destroy();

                if (ApolloSocket.this.connected) {
                    ApolloSocket.this.onclose("io client disconnect");
                }
            }
        });
        return this;
     ...
     * @return a reference to this object.
     */
    public ApolloSocket disconnect() {
        return this.close();
    }

    public ApolloSocketManager io() {
        return io;
    }
